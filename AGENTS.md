...
- **Two interchangeable provider sources.** Each exposes `.fetch() -> Snapshot` and raises `QuotaError`:
  ...
  - `quota_api.py` `Provider` (`--source standalone`, default elsewhere): uses the appliance's own OAuth credentials in `CredentialStore` under `QUOTA_HOME` (default `~/.config/quota-strip`). It refreshes tokens under a file lock and writes JSON atomically. Both Claude sources add optional limit-reset grants through `ClaudeResetGrants`, and Codex standalone adds optional reset-credit details; failures there set a snapshot warning without affecting quotas. `quota_auth.py` and `quota_callback.py` run the PKCE sign-in.
