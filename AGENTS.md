...
- **Two interchangeable provider sources.** Each exposes `.fetch() -> Snapshot` and raises `QuotaError`:
  ...
  - `quota_api.py` `Provider` (`--source standalone`, default elsewhere): uses the appliance's own OAuth credentials in `CredentialStore` under `QUOTA_HOME` (default `~/.config/quota-strip`). It refreshes tokens under a file lock and writes JSON atomically. `quota_auth.py` and `quota_callback.py` run the PKCE sign-in.
...
- Banked resets are read-only information. Never redeem a reset as part of collection, verification, or testing. Do not infer an equivalent Claude reset bank without verifying an actual source.
